...
def gg_NNLO(mh,mu,mq=mqHIGLU,fixed_NF=None):
        #formula given in mathematica notebook
        Z3=1.20206
...
def ssbar_NNLO(mh,mu,m_all_heavy_quarks=mqHIGLU):
    ms=0.0935
...
def gamma_hadrons(m,mu,m_all_heavy_quarks):
    gg=gg_NNLO_heavytop(m,mu,m_all_heavy_quarks)
    ss=ssbar_NNLO(m,mu,m_all_heavy_quarks)
    return gg+ss